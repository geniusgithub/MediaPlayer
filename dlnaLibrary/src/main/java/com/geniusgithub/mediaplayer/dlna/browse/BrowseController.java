...
public class BrowseController implements IBrowseController {
    ...
    private static final String ContentDirectory1 = "urn:schemas-upnp-org:service:ContentDirectory:1";
    private static final String Browse = "Browse";

/*
    @Override
    public boolean browseItem(Device device, String id, List<MediaItem> list) {
        org.cybergarage.upnp.Service service = device.getService(ContentDirectory1);
        if (service == null)
        {
            return false;
        }
        ...
        if (id != null){
            browseAction.setObjectID(id);
        }else{
            browseAction.setObjectID("");
        }

        browseAction.setBrowseFlag(BrowseAction.BROWSE_DIRECT_CHILDREN);
        ...
        if (browseAction.postControlAction()) {
            Argument result = browseAction.getArgument(BrowseAction.RESULT);
            Log.d(TAG, "result value = \n" + result.getValue());

            try {
                ParseUtil.parseResult(result, list);
                return true;
            }catch (Exception ex){
                ex.printStackTrace();
            }
...
*/
    ...
    public boolean browseItem(Device device, String id, List<MediaItem> list) {
        ...
        boolean parsetRet = ParseUtil.parseResult(rootNode, list);
        if (!parsetRet) {
            return false;
        }
